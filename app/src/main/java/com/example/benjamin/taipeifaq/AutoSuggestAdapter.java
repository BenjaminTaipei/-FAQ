...
public class AutoSuggestAdapter extends ArrayAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        //getView:繪製View
        View view = convertView; //convertView是系統回收的View(無法看見的View)
        if (convertView == null)
        {
    ...
    Filter nameFilter = new Filter()
    {
        ...
        @Override   //此方法用來設置過濾條件並返回過濾結果(worker thread)
        protected FilterResults performFiltering(CharSequence constraint) {
            if (constraint != null)
            {
                suggestions.clear();
                for (String names : tempItems)
                {   //String的toLowerCase：將英文字母改為小寫
                    //若儲存容器當中的資料有符合輸入的字就將這些資料加到suggestions裡面
